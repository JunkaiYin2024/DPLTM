    ...
    for i1 in range(len(r_set)): 
        for i2 in range(len(c_rate)):
            for i3 in range(len(n_set)):
                ...
                best_hyperparams = model_selection(n, r, c)
                ...
                train_delta = torch.zeros((num_sim, n // 5 * 4))
                train_risk = torch.zeros((num_sim, n // 5 * 4))
                ...
                for i in range(num_sim):
                    ...
                    train_delta[i] = train_event[0: n // 5 * 4]
                    ...
                    beta, g, risk = model(train_z, train_x)
                    risk = risk.detach()

                    beta_est[i] = beta.cpu().detach()
                    beta_sd[i]= variance(train_z, train_x, train_time, train_event, device)
                    train_risk[i] = risk - g.mean()
                    Likelihood[i] = train_loss[epoch]
                ...
                WISE = WISE_func(r, train_obs, train_delta, train_risk)
                ...
                for i in range(sim_test):
                    test_z, test_x, test_time, test_event = DataGeneration(n_test, r, c, i, 'test')
                    beta, g_hat, risk = model(test_z, test_x)
                    ICI_25[i] = ICI_func(25, risk, test_event, test_time)
                    ICI_50[i] = ICI_func(50, risk, test_event, test_time)
                    ICI_75[i] = ICI_func(75, risk, test_event, test_time)
                    RE[i] = RE_func(g_hat, test_x)
                ...
                print("c_index_mean: {:.4f}, c_index_std: {:.4f}" .format(c_index.mean().item(), c_index.std().item()))