    ...
    for i1 in range(len(r_set)): 
        for i2 in range(len(c_rate)):
            for i3 in range(len(n_set)):
                ...
                best_hyperparams = model_selection(n, r, c, 'Deep')
                ...
                for i in range(num_sim):
                    train_z, train_x, train_splines, train_derivatives, train_time, train_event = DataGeneration(n, r, c, i, num_splines, 'train', 'Deep')
                ...
                for i in range(sim_test):
                    test_z, test_x, test_splines, test_derivatives, test_time, test_event = DataGeneration(n_test, r, c, i, num_splines, 'test', 'Deep')
                    beta, theta, g_hat, risk, pred = model(test_z, test_x, test_splines)
                    RE[i] = RE_func(g_hat, test_x, 'Deep')
