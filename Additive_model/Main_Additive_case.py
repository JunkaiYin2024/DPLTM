        ...
        for i2 in range(len(c_rate)):
            for i3 in range(len(n_set)):
                ...
                for i in range(num_sim):
                    z, g0, splines, obs_t, delta = DataGeneration(n, r, c, i, num_splines, 'train', 'Additive')
                    v = np.concatenate([z, splines], axis = 0)
                    beta_ini = np.zeros(p)
                ...
                for i in range(sim_test):
                    z, g0, splines, obs_t, delta = DataGeneration(n_test, r, c, i, num_splines, 'test', 'Additive')
                    risk = np.dot(est_mean, z) + np.dot(spline_coef, splines) - np.mean(np.dot(spline_coef, splines))
                    ...
                    RE[i] = RE_func(spline_coef, splines, g0)
